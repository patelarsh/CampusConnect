...
import { LayoutDashboard, Rocket, MessageSquare, BookOpen, HelpCircle, User, LogOut, Bell, X, Check, Menu, ShieldCheck } from 'lucide-react';
        ...
        {isMobileMenuOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed top-0 left-0 w-72 h-screen bg-white z-[70] shadow-2xl flex flex-col lg:hidden"
            >
                        ...
                        <div className="max-h-[350px] overflow-y-auto">
                          {notifications.length === 0 ? (
                            ...
                            notifications.map((n, i) => (
                              <div key={`notif-${n.id || i}`} className={`p-4 border-b border-slate-50 flex items-start gap-3 hover:bg-slate-50 transition-colors ${!n.read ? 'bg-indigo-50/30' : ''}`}>
                                <div className={`w-8 h-8 rounded-lg shrink-0 flex items-center justify-center ${
                                  n.type === 'message' ? 'bg-indigo-100 text-indigo-600' : 
                                  n.type === 'mention' ? 'bg-violet-100 text-violet-600' :
                                  n.type === 'application' ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'
                                }`}>
                                   {n.type === 'message' ? <MessageSquare size={14} /> : 
                                    n.type === 'mention' ? <User size={14} /> :
