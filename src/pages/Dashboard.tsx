...
export default function Dashboard() {
  ...
  const fetchStats = async (collegeName: string) => {
    try {
      const projectsQuery = query(collection(db, 'posts'), where('collegeName', '==', collegeName));
      const notesQuery = query(collection(db, 'notes'), where('collegeName', '==', collegeName));
      const doubtsQuery = query(collection(db, 'questions'), where('collegeName', '==', collegeName));
  ...
  useEffect(() => {
    ...
    const fetchProfileAndStats = async () => {
      try {
        ...
        if (!snap.empty) {
          const collegeName = snap.docs[0].data().collegeName;
          if (collegeName) {
            fetchStats(collegeName);
          }
        }
      } catch (e) {
        console.error("Error fetching profile for stats:", e);
      }
    };
    ...
    <div className="space-y-8 pb-12">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex gap-4">
           {user?.isAdmin && (
             <Link to="/admin" className="px-6 py-3 bg-indigo-50 text-indigo-700 font-bold rounded-2xl border border-indigo-100 hover:bg-indigo-100 transition-all flex items-center gap-2">
                <ShieldCheck size={18} />
                Admin Panel
             </Link>
           )}
          ...
          <section className="bg-white border border-slate-200 rounded-[2.5rem] overflow-hidden shadow-sm">
             ...
             <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                {myProjects.length === 0 ? (
                  ...
                  myProjects.map((p) => (
                    <Link to="/projects" key={`myproj-${p.id}`} className="p-5 bg-slate-50 border border-slate-100 rounded-3xl hover:bg-slate-100 transition-all group">
              ...
              <div className="space-y-4">
                 {outgoingRequests.length === 0 ? (
                    ...
                    outgoingRequests.map((req) => (
                       <div key={`outreq-${req.id}`} className="p-4 bg-slate-800/50 rounded-2xl border border-slate-700/50">
